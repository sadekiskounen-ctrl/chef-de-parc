...
from ui.widgets import (make_primary_button, make_secondary_button,
                         make_input, make_danger_button, make_combobox)
...
class OptionsFrame(ctk.CTkFrame):
    """Écran de configuration complète."""
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            header, text='  — Réglages société, chemins d\'export, alertes et base de données',
            font=FONTS['body'],
        ...
        scroll.grid(row=2, column=0, sticky='nsew', padx=18, pady=(0, 18))
        ...
        for label, key, placeholder in societe_fields:
            ...
            make_input(frame, placeholder=placeholder, textvariable=var, width=400).pack(side='left', fill='x', expand=True)
        ...
        self.e_pdf_dir = make_input(path_row, placeholder='Chemin du dossier PDF...',
                                     textvariable=self.var_pdf_dir, width=400)
        ...
        make_input(excel_row, placeholder='Chemin du dossier Excel...',
                   textvariable=self.var_excel_dir, width=400).pack(side='left', fill='x', expand=True, padx=(0, 8))
        ...
        alert_row.pack(fill='x', padx=16, pady=8)
        ...
        make_input(row_printer, placeholder='Ex: HP LaserJet Pro',
                   textvariable=self.var_printer, width=300).pack(side='left', fill='x', expand=True, padx=(0, 8))
    ...
    def refresh(self):
        self._config = load_config()
        for key, var in getattr(self, '_soc_vars', {}).items():
            var.set(self._config.get(key, ''))
        self.var_pdf_dir.set(self._config.get('pdf_directory', ''))
        self.var_excel_dir.set(self._config.get('excel_directory', ''))
        self.var_seuil_prix.set(str(self._config.get('seuil_augmentation_prix', 15)))
        self.var_printer.set(self._config.get('imprimante', ''))
