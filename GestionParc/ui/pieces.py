...
from ui.widgets import (
    ...
    make_combobox, DatePickerWidget, bind_tree_clear_selection
)
...
class PiecesFrame(ctk.CTkFrame):
    ...
    def build(self):
        ...
        cats = ['Tous'] + get_engin_categories()
        self.filter_categorie = make_combobox(filter_bar, cats, width=150)
        self.filter_categorie.set('Tous')
        self.filter_categorie.configure(command=self._on_category_changed)
        ...
        ctk.CTkLabel(form, text='Code engin', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=0, column=3, padx=12, pady=(12, 4), sticky='w')
        engins_codes = [e['code_engin'] for e in get_all_engins()]
        self.e_code_engin = make_combobox(form, engins_codes if engins_codes else ['CL-001'], width=180)
        self.e_code_engin.grid(row=1, column=3, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_code_engin)

        # Ligne 2
        ctk.CTkLabel(form, text='Famille', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=2, column=0, padx=12, pady=(0, 4), sticky='w')
        self.combo_famille = make_combobox(form, FAMILLES_PIECES, width=200)
        self.combo_famille.grid(row=3, column=0, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.combo_famille)

        # Champ Date avec Calendrier Popup
        ctk.CTkLabel(form, text='Date livraison', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=2, column=1, padx=12, pady=(0, 4), sticky='w')
        self.e_date = DatePickerWidget(form, placeholder='2026-01-15', width=180)
        self.e_date.grid(row=3, column=1, padx=12, pady=(0, 10), sticky='w')

        ctk.CTkLabel(form, text='Quantité', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=2, column=2, padx=12, pady=(0, 4), sticky='w')
        self.e_qte = make_input(form, '8', 120)
        self.e_qte.grid(row=3, column=2, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_qte)

        ctk.CTkLabel(form, text='Emplacement', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=2, column=3, padx=12, pady=(0, 4), sticky='w')
        self.e_emplacement = make_input(form, 'Magasin A', 180)
        self.e_emplacement.grid(row=3, column=3, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_emplacement)

        # Ligne 3
        ctk.CTkLabel(form, text='Unité', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=4, column=0, padx=12, pady=(0, 4), sticky='w')
        self.combo_unite = make_combobox(form, ['JEUX', 'UNITE', 'LITRE'], width=150)
        self.combo_unite.grid(row=5, column=0, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.combo_unite)

        ctk.CTkLabel(form, text='Ancien prix', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=4, column=1, padx=12, pady=(0, 4), sticky='w')
        self.e_ancien = make_input(form, '100', 140)
        self.e_ancien.grid(row=5, column=1, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_ancien)

        ctk.CTkLabel(form, text='Nouveau prix', font=FONTS['body_bold'], text_color=COLORS['text_primary']).grid(row=4, column=2, padx=12, pady=(0, 4), sticky='w')
        self.e_nouveau = make_input(form, '120', 140)
        self.e_nouveau.grid(row=5, column=2, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_nouveau)

        ctk.CTkLabel(form, text='Stock Alerte (Seuil min)', font=FONTS['body_bold'], text_color='#DC2626').grid(row=4, column=3, padx=12, pady=(0, 4), sticky='w')
        self.e_seuil = make_input(form, '5', 120)
        self.e_seuil.grid(row=5, column=3, padx=12, pady=(0, 10), sticky='w')
        self.form_fields.append(self.e_seuil)

        # ── Boutons ──────────────────────────────────────────────────────────
        btn_row = ctk.CTkFrame(form, fg_color='transparent')
        btn_row.grid(row=6, column=0, columnspan=4, padx=12, pady=(4, 12), sticky='w')
        ...
        self.tree = ttk.Treeview(table_frame,
            columns=('ref', 'fournisseur', 'designation', 'engin', 'famille', 'qte', 'seuil', 'prix'),
        ...
        self.tree.heading('engin', text='Engin')
        ...
        for col in ['ref', 'fournisseur', 'designation', 'engin', 'famille', 'qte', 'seuil', 'prix']:
    ...
    def _on_category_changed(self, choice=None):
        cat = self.filter_categorie.get().strip()
        marques = ['Toutes'] + get_engin_designations_by_category(cat)
    ...
    def add_piece(self):
        ...
        if not all([ref, code_engin, famille]):
            messagebox.showwarning('Attention', 'Référence interne, code engin et famille sont obligatoires.')
            return
        engin = get_engin_by_code(code_engin)
        if not engin:
            messagebox.showerror('Erreur', f'Code engin "{code_engin}" introuvable.')
            return
        data = {
    ...
    def update_selected_piece(self):
        ...
        if not ref:
            ...
            return
        engin = get_engin_by_code(code_engin) if code_engin else None
    ...
    def on_row_select(self, event):
        ...
        for piece in get_all_pieces():
            if piece['reference_interne'] == values[0]:
                ...
                if piece.get('code_engin'):
                    self.e_code_engin.set(piece['code_engin'])
    ...
    def refresh(self):
        ...
        filter_cat = self.filter_categorie.get().strip().lower() if hasattr(self, 'filter_categorie') else 'tous'
        ...
        for piece in get_all_pieces():
            ...
            # Filtre par catégorie engin
            if filter_cat not in ('tous', 'toutes', ''):
                engin_cat = str(piece.get('engin_categorie', '') or '').lower()
                if filter_cat not in engin_cat and engin_cat not in filter_cat:
                    continue
